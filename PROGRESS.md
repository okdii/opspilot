...
✅ Security command center — Security tab split into Threat Intelligence + Fail2ban sub-tabs; Threat Intelligence command-center layout (status bar with server-side aggregate chips, needs-approval banner, threat feed + response ledger); server-side pagination (50/page) with reusable Pager component and dedicated /security/summary endpoint
